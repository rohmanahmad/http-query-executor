...
export default {
    ...
    uploadTargetUserCallbackClient(res) {
        ...
        if (status === 'finished') {
            this.isFinishedProcessing = true
            this.validateRequiredFields()
        } else {
            this.isFinishedProcessing = false
        }
    },
    ...
    async saveNotification(e, status) {
        try {
            ...
            console.log('progress', this.stillProgress)
            if (this.stillProgress) return logInfo('Still progress')
            ...
            if (this.isEditMode) {
                ...
                if (status === 'WAITING_FOR_APPROVAL') this.buttons.waiting_for_approval.loading = true
                else this.buttons.update_changes.loading = true
                this.update()
    ...
    validateRequiredFields() {
        ...
        try {
            ...
            if (this.invalidUrl) throw new Error('Invalid URL')
            this.buttons.waiting_for_approval.disabled = false
            this.buttons.save_as_draft.disabled = false
            this.buttons.update_changes.disabled = false
        } catch (err) {
            ...
            this.buttons.save_as_draft.disabled = true
            this.buttons.update_changes.disabled = true
            this.validateErrorMessage = err.message
        }
        this.update()
    },
    validateUrl(e) {
        ...
        this.validUrl = false
        // if (targetValue.indexOf(validatorText) > -1) {
        //     if (targetValue.replace(validatorText, '').length > 5) this.validUrl = true
        //     else this.invalidUrl = true
        // } else this.invalidUrl = true
        if (/https\:\/\/[a-zA-Z0-9][a-zA-Z0-9-]{1,61}[a-zA-Z0-9](?:\.[a-zA-Z]{2,})+/.test(targetValue)) {
            this.validUrl = true
        } else this.invalidUrl = true
