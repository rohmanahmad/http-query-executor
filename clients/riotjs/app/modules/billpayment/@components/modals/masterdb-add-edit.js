...
export default {
    ...
    callbackPaymentMethod(res) {
        if(this.editMode == true){
            if (res.code !== this.codeTtc){
                this.payload.ttc = this.codeTtc
                this.codeTtc = res.code
            }else{
                this.payload.ttc = this.codeTtc
            }
        }else{
    ...
    callbackCounterpartyBankName(res){
        ...
        const patternKey = [this.payload.partnerName, dataValue.replace(/i /g, '')].join('_').replace(/ /g, '_').toLowerCase()
        if (patternKey && patternMethod[patternKey]) {
            ...
            if (patternKey === 'artajasa_bank_indonesia') {
                this.payload.settlementPaymentMethod = "RTGS"
